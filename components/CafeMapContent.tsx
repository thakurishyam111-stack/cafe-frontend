...
const createPinIcon = (color: string, label: string) =>
 ...
 const cafeIcon = useMemo(
    () => createPinIcon("#f59e0b", "☕"),
    []
  );

  const userIcon = useMemo(
    () => createPinIcon("#2563eb", "●"),
    []
  );

export default function CafeMapContent() {
  ...
  const requestLocation = () => {
    ...
    const onError = () => {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setLocationError(
            "Location permission is blocked or unavailable. You can still view the cafe location."
          );
          setIsLocating(false);
        },
        {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] bg-slate-950 px-4 py-10 text-slate-100 shadow-2xl sm:rounded-[3rem] sm:px-6 lg:px-8 lg:py-14">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(251,191,36,0.16),transparent_35%)]" />
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mb-8 text-center md:mb-10">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base">
            This map shows the cafe location, your current
            position when available, and a simple route view
            like Google Maps.
          </p>
        </div>
          ...
          <div className="flex flex-col justify-between rounded-[1.7rem] border border-slate-800 bg-slate-900/80 p-6 shadow-xl backdrop-blur">
            ...
            <div className="mt-6 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-100">
              {isLocating
                ...
                : locationError ||
                  "Your location is shown when available."}
            </div>
