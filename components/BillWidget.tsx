...
export default function BillWidget({ isOpen, onClose }: BillWidgetProps) {
  ...
  const fetchBill = async (): Promise<void> => {
    ...
    try {
      ...
      const matchedOrders = allOrders.filter((ord: Order): boolean => {
  ...
  const unpaidBills = ordersList.filter((ord: Order): boolean => {
  ...
  const grandTotal = unpaidBills.reduce(
    (sum: number, ord: Order): number => sum + (Number(ord.total) || 0),
    0,
  );
  ...
  const handleBulkPayment = async (method: PaymentMethod): Promise<void> => {
    ...
    // CASH
    if (method === "Cash") {
      alert("Please go to Cashier Department for Cash payment...");
      return;
    }
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          ...
          <div className="bg-slate-800/40 p-4 rounded-2xl border border-slate-800/80">
            <div className="space-y-3">
              {message && (
                <p className="text-red-400 text-xs mt-2.5 text-center bg-red-100/10 p-2 rounded-lg border border-red-500/20">
                  {message}
                </p>
              )}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        ...
                        <span className="px-1.5 py-0.5 rounded text-[9px] uppercase font-bold tracking-wider bg-red-600/20 text-red-400 border border-red-500/20">
                    ...
                    {isVisibleStatus ? (
                      <>
                        ...
                        <hr className="my-2.5 border-slate-200/60" />

                        <div className="flex justify-between font-bold text-xs text-blue-400">
                          <span >Bill Total</span>

                          <span className="text-green-400">Rs. {Number(order.total || 0)}</span>
                        </div>
                      </>
                    ) : (
          ...
          <div className="p-4 bg-slate-950 border-t border-slate-800 rounded-t-2xl shadow-inner">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                disabled={paying}
                className="text-[11px] font-bold text-slate-900 bg-gray-300 hover:bg-slate-100 py-2.5 px-1 rounded-xl transition-all disabled:opacity-50 cursor-pointer text-center"
              >
